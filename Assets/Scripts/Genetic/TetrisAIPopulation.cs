...
public class TetrisAIPopulation : MonoBehaviour, IPopulation<GameManager>
{
    ...
    public void BreedNewPopulation()
    {
        ...
        {
            if (Random.Range(0, 1.0f) < .05f) Mutate(gm);
        }
        generationCounter++;
    }
    ...
    public float Fitness(GameManager dna)
    {
        ...
        return (float) (4*dna.moveCounter - dna.GridH() - dna.GetHeights().Sum()/w + (80.0f*dna.linesDeleted) - .5f*dna.GetHoles() - 2*dna.GetBumpines());
    }
